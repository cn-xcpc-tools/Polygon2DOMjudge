...
from .typing import Config, ValidatorFlags, Result, Results
...
class Polygon2DOMjudge:
    ...
    def _add_jury_solutions(self) -> Polygon2DOMjudge:
        ...
        for solution in self._problem.solutions:
            ...
            result = self._config['tag'].get(tag, 'check_manually')

            if isinstance(result, str):
                result_dir = self.temp_dir / 'submissions' / result
                if result == 'check_manually':
                    logger.warning(
            ...
            if (source := solution.find('source[@path][@type]')) is not None:
                ...
                self._add_solutions_with_expected_result(src, dst, lang, result)

        return self

    def _add_solutions_with_expected_result(self, src: Path, dst: Path, lang: str,  result: Result | Results) -> None:
        if isinstance(result, str):
            logger.info(f'- {src.name}: Expected result: {result}')
        ...
        with open(src, 'r') as f:
            ...
            if '@EXPECTED_RESULTS@' in content or '@EXPECTED_SCORE@' in content:
                ...
                logger.info(
                    f'- {src.name}: Expected result: {",".join(map(lambda x: PROBLEM_RESULT_REMAP[x.upper()].lower(), result))}')
                with open(dst, 'w') as f:
                    ...
                    if comment_str := self._config['comment_str'].get(lang, None):
                        ...
                        f.write(
                            f'{comment_str} @EXPECTED_RESULTS@: {",".join(map(lambda x: PROBLEM_RESULT_REMAP.get(x.upper(), x.upper()), result))}\n')
                    else:
